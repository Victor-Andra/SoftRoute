...
// 👉 LOG GLOBAL NO INÍCIO DO ARQUIVO (para garantir que aparece)
console.log("🚀 [SERVER.JS] Módulo de rotas carregado - /login/backup registrado");

router.post('/login/backup', 
    // 👉 LOG ANTES DO PASSPORT (para confirmar que a rota foi atingida)
    (req, res, next) => {
        console.log("🔗 [DEBUG] Rota /login/backup atingida - Method:", req.method, "| Body:", JSON.stringify(req.body));
        next();
    },
    passport.authenticate('local', {
        failureRedirect: '/menu/login',
    ...
    async function (req, res) {
        ...
        console.log("🔐 [LOGIN/BACKUP] INÍCIO - Autenticação bem-sucedida");
        ...
        try {
            // ========================================================================
            // 🔐 FASE 2: Validar Usuário
            // ========================================================================
            console.log("\n🔐 [FASE 2] Buscando usuário no banco...");
            ...
            console.log(`   🔍 Resultado da busca: ${usu ? 'ENCONTRADO' : 'NÃO ENCONTRADO'}`);
            
            if (!usu || usu.usuario_status !== "Ativo") {
                console.log(`   ❌ Falha: usuário "${req.body.email}" não encontrado ou inativo`);
                req.flash("error_message", "Usuário ou senha inválidos ou inativo.");
                return res.redirect('/menu/login');
            }

            console.log(`   ✅ Usuário validado: ${usu.usuario_nome} (ID: ${usu._id})`);
            console.log(`   👤 Perfil: ${usu.usuario_perfilid} | Nome Abrev: ${usu.usuario_nomeabrev || '(não definido)'}`);

            const perfilId = usu.usuario_perfilid;
            const idUsu = usu._id;

            // Cookies de sessão
            const perfisAdmin = ["62421801a12aa557219a0fb9", "62421857a12aa557219a0fc1", "624218f5a12aa557219a0fd0"];
            const tempoCookie = perfisAdmin.includes(perfilId) 
                ? (5 * 60 * 60 * 1000) 
                : (2 * 60 * 60 * 1000);
            ...
            console.log(`   🍪 Cookies definidos: lvlUsu=${perfilId}, idUsu=${idUsu}`);

            // ========================================================================
            // 🎂 FASE 3: Aniversariantes → Widget "percep"
            // ========================================================================
            console.log("\n🎂 [FASE 3] Carregando aniversariantes → Widget #percep");
            ...
            const semanaDias = Array.from({ length: 7 }).map((_, i) => {
                ...
                return { dia: String(d.getUTCDate()).padStart(2, '0'), mes: String(d.getUTCMonth() + 1).padStart(2, '0') };
            });
            ...
            console.log(`   👥 Colaboradores: ${aniversariantesDaSemanaUsuario.length} | 🧒 Beneficiários: ${aniversariantesDaSemanaBene.length}`);

            // ========================================================================
            // 📋 FASE 4: Agendas → Widgets "Faltaevo" + "Pontage"
            // ========================================================================
            console.log("\n📋 [FASE 4] Carregando agendas semanais");
            ...
            console.log(`   📦 Agendas brutas: ${agendasBrutas.length}`);

            const [salas, terapias, benesFull] = await Promise.all([
                Sala.find(), Terapia.find(), Bene.find()
            ]);

            // Filtrar substituições
            let arrIdsAgendas = agendasBrutas.map(a => a._id);
            const agendasSemanais = await Agenda.find({ agenda_tempId: { $in: arrIdsAgendas } });
            
            agendaFinal = agendasBrutas.filter(a => {
                let match = agendasSemanais.find(s => s.agenda_tempId?.toString() === a._id?.toString());
                if (!match) return true;
                return a.agenda_usuid?.toString() === match.agenda_usuid?.toString();
            });
            console.log(`   ✅ Após filtro substituição: ${agendaFinal.length}`);

            // Montar evolucaoFaltante
            ...
            console.log(`   📋 evolucaoFaltante: ${evolucaoFaltante.length} pendentes`);

            // ========================================================================
            // 📦 FASE 5: Dados Globais
            // ========================================================================
            console.log("\n📦 [FASE 5] Carregando dados globais");
            ...
            // ========================================================================
            console.log("\n🎬 [FASE 7] Renderizando view 'branco.handlebars'");
            console.log(`✅ [RESUMO] Dados enviados:`);
            console.log(`   • flash: "${flash.texto}"`);
            console.log(`   • usuario_nomeabrev: "${usu.usuario_nomeabrev}"`);
            console.log(`   • Widget #Faltaevo: ${agendaFinal.length} agendas`);
            console.log(`   • Widget #Pontage: ${evolucaoFaltante.length} pendentes`);
            console.log(`   • Widget #percep: ${aniversariantesDaSemanaBene.length} bene + ${aniversariantesDaSemanaUsuario.length} usuarios`);
            console.log("=".repeat(80));
            
            res.render("branco", {
                flash,
                // 👉 NOVO: Nome abreviado do usuário logado (para usar na view)
                usuario_nomeabrev: usu.usuario_nomeabrev || usu.usuario_nome,
