...
router.post('/login/backup', passport.authenticate('local', {
    ...
    try {
        ...
        const idUsu = usu._id.toString();  // 👈 Garantir que é string
        ...
        res.cookie('idUsu', usu._id, { expires: new Date(Date.now() + tempoCookie) });
...
async function login(req, res, dbEscolhida) { // Processa após verificação de credenciais
    try {
        ...
        }, 'agenda_data agenda_usuid agenda_categoria agenda_temp agenda_tempId agenda_salaid agenda_beneid agenda_obs agenda_terapiaid agenda_selo').then((agenda) => {
...
// =======================================================
// 🔵 FASE FINAL: REDEFINIÇÃO (Evolução já realizada)
// =======================================================
function redefinicaoEvolucao(agendas) {
    console.log("🔵 [redefinicao] Iniciando pós-processamento de evolução...");

    return agendas.map((a, index) => {

        try {
            const selo = a.agenda_selo;

            // 👉 Trata boolean e string ("true")
            const evoluido = (
                selo === true ||
                selo === "true" ||
                selo === 1 ||
            ...
            if (evoluido) {
                console.log(`✅ [redefinicao] Agenda ${index} (${a._id}) já evoluída`);

                // 🔥 SOBRESCREVE COMPLETAMENTE A UI
                a.ui = {
                    icone: 'check',
                    tooltipTitulo: 'Evolução',
                    tooltipTexto: 'Evolução já realizada',
                    temLink: false
                };

                // 👉 Opcional: flag auxiliar (debug / futuro)
                a._evolucaoProcessada = true;
            }

        } catch (err) {
            console.error(`❌ [redefinicao] Erro na agenda ${index}:`, err);
        }
