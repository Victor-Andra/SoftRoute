...
const UsuarioSchema = mongoose.Schema({
  usuario_login : { type: String, required: true, unique: true },
  ...
  usuario_cpf : { type: String, required: false, unique: true },
  ...
  usuario_funcaoid : { type: String, required: true },
...
module.exports = {
  ...
  UsuarioSchema,
   ...
   usuarioAdicionar: async (req, res) => {
    try {
      let usuarioExiste = await UsuarioModel.findOne({ usuario_nome: req.body.usuarioNome });
      let dataAtual = new Date();
  
      if (usuarioExiste) {
        return res.send("O nome do usuario já existe");
      }
