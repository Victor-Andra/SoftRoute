...
module.exports = {FiltroEvoatend,
    ...
    console.log('[BACKEND] Body recebido:', JSON.stringify(req.body, null, 2)); // Log do payload
    ...
    try {
        ...
        console.log('[BACKEND] Session MongoDB iniciada');
        ...
        const { 
            ...
            // guialote_dataenvio não é obrigatório, então pode ficar sem
        } = req.body;

        const idUsu = req.cookies['idUsu'];
        const agora = new Date();

        if (!listaAgendaIds || listaAgendaIds.length === 0) {
            throw new Error("Nenhum agendamento selecionado para o lote.");
        }
        console.log(`[BACKEND] Validando ${listaAgendaIds.length} agendas...`);

        // 1. VALIDAÇÃO PRÉVIA
        const agendasCandidatas = await Agenda.find({ 
            _id: { $in: listaAgendaIds } 
        }).session(session);
        
        console.log(`[BACKEND] Encontradas ${agendasCandidatas.length} agendas no banco`);

        if (agendasCandidatas.length !== listaAgendaIds.length) {
            throw new Error("Alguns agendamentos selecionados não foram encontrados.");
        }
        ...
        for (const agenda of agendasCandidatas) {
            const temGuia = agenda.agenda_guia && agenda.agenda_guia.guia_num && agenda.agenda_guia.guia_num.trim() !== '';
            const temSenha = agenda.agenda_guia && agenda.agenda_guia.guia_senha && agenda.agenda_guia.guia_senha.trim() !== '';
            const jaTemLote = agenda.agenda_loteid != null && agenda.agenda_loteid != undefined;

            if (!temGuia || !temSenha) {
                errosValidacao.push(`Agenda ${agenda._id}: Falta Guia ou Senha.`);
            } else if (jaTemLote) {
                errosValidacao.push(`Agenda ${agenda._id}: Já pertence a outro lote.`);
            } else {
        ...
        if (errosValidacao.length > 0) {
            console.warn('[BACKEND] Falha na validação:', errosValidacao);
            throw new Error("Validação falhou: " + errosValidacao.join("; "));
        }
        console.log(`[BACKEND] ${idsValidos.length} agendas aprovadas para lote.`);

        // 2. CRIAÇÃO DO LOTE
        console.log('[BACKEND] Criando documento Guialote...');
        ...
        await novoLote.save({ session });
        console.log(`[BACKEND] Lote salvo com ID: ${novoLote._id}`);

        // 3. VINCULAR AGENDAMENTOS
        console.log('[BACKEND] Atualizando agendas com agenda_loteid...');
        const updateResult = await Agenda.updateMany(
            { _id: { $in: idsValidos } },
            { 
                $set: { 
                    agenda_loteid: novoLote._id,
        ...
        console.log(`[BACKEND] Agendas atualizadas: ${updateResult.modifiedCount}`);

        await session.commitTransaction();
        console.log('[BACKEND] Transação commitada com sucesso!');
        
        return res.json({ 
            ok: true, 
            message: `Lote criado com sucesso! ${idsValidos.length} agendamentos vinculados.`,
            loteId: novoLote._id 
        });

    } catch (err) {
        console.error("❌ [BACKEND] ERRO AO CRIAR LOTE:", err);
        if (session) await session.abortTransaction();
        return res.status(400).json({ ok: false, message: err.message });
    } finally {
        if (session) session.endSession();
        console.log('[BACKEND] Session encerrada.');
    }
}
}