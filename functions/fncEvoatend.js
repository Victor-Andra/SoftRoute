...
async apagarEvolucaoIndevidaevoatend(req, res) {
    ...
    listaEvoatendgeral(req, res, resposta) {
       ...
       Usuario.find({
            ...
            // Ordena os usuários por nome (removendo acentos para comparação correta)
            usuario.sort((a, b) => {
            ...
            console.log("Tamanho de usuários: " + usuario.length);

            Ano.find().sort({ ano_nome: 1 }).then((ano) => {
                Bene.find({ bene_status: "Ativo" }).then((bene) => {
                    // Ordena os beneficiários por nome (removendo acentos)
                    bene.sort((a, b) => {
    ...
    async filtraEvoatendgeral(req, res) {
        ...
        // Na captura dos filtros:
            ...
            // Adiciona todas as agendas semanais
            agendaFinal.push(...agendaS);

            // Pega os agenda_tempId das semanais, exceto Supervisão
            const idsTemporarios = agendaS
                .filter(a => a.agenda_categoria?.toLowerCase() !== "Supervisão")
                .map(a => a.agenda_tempId?.toString())
                .filter(id => id);

            // Adiciona as fixas que não foram substituídas por uma semanal
            agenda.forEach(agenda => {
                const idFixa = agenda._id?.toString();

                if (!idsTemporarios.includes(idFixa)) {
                    agendaFinal.push(agenda);
                }
            });
            ...
            // Remove agendas temporárias e ordena
            var agendasFiltradas = agendaFinal.sort((a, b) => (a.agenda_benenome || "").localeCompare(b.agenda_benenome || "", 'pt-BR'));
    ...
    async filtraEvoatendgeral_Victor(req, res) {
        try {
            // ===== CONFIGURAÇÃO =====
            const db = req.cookies['preferredDb'];
            const Agenda = getModel(db, 'tb_agenda', agendaClass.AgendaSchema);
        ...
        // Na captura dos filtros:
            ...
            try {
                agendaS = await Agenda.find({
                    $or: [
                        {
                            ...buscaS // Filtros normais (agenda_temp = true + data/pessoa/selo)
                        },
                        {
                            ...buscaS, // ✅ CORREÇÃO: Aplica os mesmos filtros aqui também!
                            agenda_tempId: {
                                $in: agendaTempIds
                            }
                        }
