...
module.exports = {
    listaGrafprog(req, res){
        ...
        Grafprog.find().then((grafprog) =>{
            ...
            res.render('area/aba/grafprog/grafprogLis', {Grafprogs: grafprog, Usuarios: usuario, Benes: bene})
        })})}).catch((err) =>{
            console.log(err)
                    ...
                    console.log("Listagem Realizada de Usuário")
                        Bene.find().sort({bene_nome: 1}).then((bene)=>{
                            console.log("Listagem Realizada de beneficiarios")
                                res.render("area/aba/grafprog/grafprogCad", {convs: conv, terapias: terapia, usuarios: usuario, benes: bene})
        })})})}).catch((err) =>{
            console.log(err)
            ...
            Terapia.find().then((terapia)=>{
                ...
                Usuario.find({"usuario_funcaoid":"6241030bfbcc51f47c720a0b", "usuario_status":"Ativo"}).then((usuario)=>{//Usuário c/ filtro de função = Terapeutas
                    console.log("Listagem Realizada de Usuário")
                        Bene.find().sort({bene_nome: 1}).then((bene)=>{
                            console.log("Listagem Realizada de beneficiarios")
                                res.render("area/aba/grafprog/grafprogEdi", {convs: conv, terapias: terapia, usuarios: usuario, benes: bene})
        })})})}).catch((err) =>{
            console.log(err)
    ...
    deletaGrafprog(req,res){
        Grafprogfisio.deleteOne({_id: req.params.id}).then(() =>{
            Conv.find().then((conv)=>{
                ...
                res.render('area/aba/grafprog/grafprogLis', {convs: conv, terapias: terapia, usuarios: usuario, benes: bene, flash})
            })})})}).catch((err) =>{
                console.log(err)
