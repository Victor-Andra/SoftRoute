...
module.exports = {
    listaCars(req, res){
        ...
        Cars.findOne().then((cars) =>{
            ...
            res.render('area/aba/cars/carsLis', {Carss: cars, Terapeutas: terapeuta, Benes: bene})
        })})}).catch((err) =>{
            console.log(err)
        ...
        Bene.find().then((bene)=>{
            ...
            //console.log("Listagem Realizada de Beneficiários!")
                Usuario.find({usuario_funcaoid:"6241030bfbcc51f47c720a0b"}).then((terapeuta)=>{//Usuário c/ filtro de função = Terapeutas
                    terapeuta.sort((a,b) => (a.usuario_nome > b.usuario_nome) ? 1 : ((b.usuario_nome > a.usuario_nome) ? -1 : 0));//Ordena por ordem alfabética 
                    //console.log("Listagem Realizada de Usuário")
                            res.render("area/aba/cars/carsCad", {Benes: bene, Terapeutas: terapeuta})
        })}).catch((err) =>{
            console.log(err)
            ...
            Terapia.find().then((terapia)=>{
                ...
                Usuario.find({usuario_funcaoid:"6241030bfbcc51f47c720a0b"}).then((terapeuta)=>{//Usuário c/ filtro de função = Terapeutas
                    ...
                    Bene.find().then((bene)=>{
                        bene.sort((a,b) => (a.bene_nome > b.bene_nome) ? 1 : ((b.bene_nome > a.bene_nome) ? -1 : 0));//Ordena por ordem alfabética 
                        //console.log("Listagem Realizada de Beneficiários!")
                                res.render("area/aba/cars/carsEdi", {Convs: conv, Terapias: terapia, Terapeutas: terapeuta, Benes: bene})
        })})})}).catch((err) =>{
            console.log(err)
    ...
    deletaCars(req,res){
        Carsfisio.deleteOne({_id: req.params.id}).then(() =>{
            Conv.find().then((conv)=>{
                ...
                res.render('area/aba/cars/carsLis', {convs: conv, terapias: terapia, usuarios: usuario, benes: bene, flash})
            })})})}).catch((err) =>{
                console.log(err)
