            ...
            Conv.find().then((conv)=>{
                Conv.find({_id: req.body.relConvid}).then((c)=>{
                    ...
                    Terapia.find().then((terapia)=>{
                        ...
                        Ano.find().then((ano)=>{
                            res.render("atendimento/relatendval", {terapias: terapia, anos: ano, convs: conv, rels: rel, total, periodoDe, periodoAte, conv_nome})
                        })
                    })
                })
            })
                            ...
                            Bene.findOne({_id: req.body.relBeneid}).then((b)=>{
                                ...
                                cc.then((convcre)=>{
                                    Terapia.find().then((terapia)=>{
                                        ...
                                        res.render("atendimento/relatendvalnf", {terapias: terapia, anos: ano, convimps: convimp, benes: bene, rels: rel, total, periodoDe, periodoAte, bene_nome, bene_retem, bene_doc, bene_tomador, conv_nome})
                                    })
                                })
                            })
                        })
...
//testar com o nextNumAtendCopiar string e int
                        ...
                        Convdeb.find().then((convdeb) => {
                            ...
                            Usuario.find({usuario_funcaoid:"6241030bfbcc51f47c720a0b"}).then((terapeuta)=>{
                                ...
                                Terapia.find().then((terapia)=>{
                                    ...
                                    res.render("atendimento/atendreltera/relatendterapiacons", {terapeutas: terapeuta, anos: ano, terapias: terapia, benes: bene, rels: rel, periodoDe, periodoAte, terapeuta_nome, pesquisa})
                                })
                                })
                            })
                        })
    ...
    relAtendteraana(req,res){
        let db = req.cookies['preferredDb'];
        Ano = getModel("SoftRoute", 'tb_ano', anoClass.AnoSchema)
        Bene = getModel(db, 'tb_bene', beneClass.BeneSchema)
        Terapia = getModel(db, 'tb_terapia', terapiaClass.TerapiaSchema)
        ...
        Bene.find().then((bene)=>{
            Ano.find().then((ano)=>{
                Terapia.find().then((terapia)=>{
                    Usuario.find({usuario_funcaoid:"6241030bfbcc51f47c720a0b"}).then((terapeuta)=>{
                        terapeuta.sort((a,b) => ((a.usuario_nome.normalize('NFD').replace(/[\u0300-\u036f]/g, "")) > (b.usuario_nome.normalize('NFD').replace(/[\u0300-\u036f]/g, ""))) ? 1 : (((b.usuario_nome.normalize('NFD').replace(/[\u0300-\u036f]/g, "")) > (a.usuario_nome.normalize('NFD').replace(/[\u0300-\u036f]/g, ""))) ? -1 : 0));//Ordena o bene por nome//Ordena por ordem alfabética 
                        res.render("atendimento/atendreltera/relatendteraana", {Rels: rel, anos: ano, terapeutas: terapeuta, terapias: terapia, benes: bene})
        })})})}).catch((err) =>{
            console.log(err)
        })
    },
...
//NEW END
    ...
    relatendgestaoana: async (req, res) => {
        try {
           ...
           // Garante que seja string, mesmo se vier como array (evita "Invalid Date")
            const dataIniStr = Array.isArray(req.query.dataIni) 
                ? req.query.dataIni[0] 
                : (req.query.dataIni || req.body.dataIni);

            const dataFimStr = Array.isArray(req.query.dataFim) 
                ? req.query.dataFim[0] 
                : (req.query.dataFim || req.body.dataFim);
            ...
            const pipeline = [
                ...
                // 2. Converter campos de valor de string para número
                {
                    $addFields: {
                        atend_valorcre_num: {
                            $cond: {
                                if: { $ne: ["$atend_valorcre", ""] },
                                then: { $toDouble: { $replaceAll: { input: "$atend_valorcre", find: ",", replacement: "." } } },
                                else: 0
                            }
                        },
                        atend_valordeb_num: {
                            $cond: {
                                if: { $ne: ["$atend_valordeb", ""] },
                                then: { $toDouble: { $replaceAll: { input: "$atend_valordeb", find: ",", replacement: "." } } },
                                else: 0
                            }
                        },
                        atend_mergevalorcre_num: {
                            $cond: {
                                if: { $and: [{ $ne: ["$atend_mergevalorcre", null] }, { $ne: ["$atend_mergevalorcre", ""] }] },
                                then: { $toDouble: { $replaceAll: { input: "$atend_mergevalorcre", find: ",", replacement: "." } } },
                                else: 0
                            }
                        },
                        atend_mergevalordeb_num: {
                            $cond: {
                                if: { $and: [{ $ne: ["$atend_mergevalordeb", null] }, { $ne: ["$atend_mergevalordeb", ""] }] },
                                then: { $toDouble: { $replaceAll: { input: "$atend_mergevalordeb", find: ",", replacement: "." } } },
                                else: 0
                            }
                        },
                        atend_fixovalorcre_num: {
                            $cond: {
                                if: { $and: [{ $ne: ["$atend_fixovalorcre", null] }, { $ne: ["$atend_fixovalorcre", ""] }] },
                                then: { $toDouble: { $replaceAll: { input: "$atend_fixovalorcre", find: ",", replacement: "." } } },
                                else: 0
                            }
                        },
                        atend_fixovalordeb_num: {
                            $cond: {
                                if: { $and: [{ $ne: ["$atend_fixovalordeb", null] }, { $ne: ["$atend_fixovalordeb", ""] }] },
                                then: { $toDouble: { $replaceAll: { input: "$atend_fixovalordeb", find: ",", replacement: "." } } },
                                else: 0
                            }
                        }
                    }
                ...
                // 3. Lookup: Beneficiário
                {
                    $lookup: {
                        from: Bene.collection.name,
                        localField: "atend_beneid",
                        foreignField: "_id",
                        as: "bene"
                    }
                },
                ...
                // 5. Lookup: Terapeuta principal
                {
                    $lookup: {
                        from: Usuario.collection.name,
                        localField: "atend_terapeutaid",
                        foreignField: "_id",
                        as: "terapeuta"
                    }
                },
                ...
                {
                    $lookup: {
                        from: Sala.collection.name,
                        localField: "atend_salaid",
                        foreignField: "_id",
                        as: "sala"
                    }
                },
                ...
                // 8. Lookup: Merge Terapeuta
                {
                    $lookup: {
                        from: Usuario.collection.name,
                        localField: "atend_mergeterapeutaid",
                        foreignField: "_id",
                        as: "mergeterapeuta"
                    }
                },
                ...
                // 9. Lookup: Merge Terapia
                {
                    $lookup: {
                        from: Terapia.collection.name,
                        localField: "atend_mergeterapiaid",
                        foreignField: "_id",
                        as: "mergeterapia"
                    }
                },
                ...
                // 10. Lookup: Fixo Terapeuta
                {
                    $lookup: {
                        from: Usuario.collection.name,
                        localField: "atend_fixoterapeutaid",
                        foreignField: "_id",
                        as: "fixoterapeuta"
                    }
                },
                ...
                // 11. Lookup: Fixo Terapia
                {
                    $lookup: {
                        from: Terapia.collection.name,
                        localField: "atend_fixoterapiaid",
                        foreignField: "_id",
                        as: "fixoterapia"
                    }
                },
                ...
                // 12. Projeto final: substituir IDs por nomes e remover campos sensíveis
                {
                    $project: {
                        _id: 0,
                        atend_atenddata: 1,
                        ...
                        // Valores convertidos
                        atend_valorcre_num: { $round: ["$atend_valorcre_num", 2] },
                        atend_valordeb_num: { $round: ["$atend_valordeb_num", 2] },
                        atend_mergevalorcre_num: { $round: ["$atend_mergevalorcre_num", 2] },
                        atend_mergevalordeb_num: { $round: ["$atend_mergevalordeb_num", 2] },
                        atend_fixovalorcre_num: { $round: ["$atend_fixovalorcre_num", 2] },
                        atend_fixovalordeb_num: { $round: ["$atend_fixovalordeb_num", 2] },
            ...
            const relsFormatado = rels.map(r => ({
                ...r,
                dt: fncGeral.getData(r.atend_atenddata), // dd/mm/yyyy
                hora: r.atend_atendhora || ''
            }));
            
            // Carrega terapeutas para o filtro (opcional)
            const terapeutas = await Usuario.find({ usuario_funcaoid: "6241030bfbcc51f47c720a0b" }).sort({ usuario_nome: 1 });

            // Na renderização (final da função):
            res.render("atendimento/atendreltera/gestao/relatendgestaoana", {
                rels: relsFormatado,
                terapeutas,
                periodoDe: fncGeral.getDataInvert(dataIni.toISOString().substring(0, 10)), // dd/mm/yyyy
                periodoAte: fncGeral.getDataInvert(dataFim.toISOString().substring(0, 10)), // dd/mm/yyyy
                pesquisa: {
                    dataIni: dataIni.toISOString().substring(0, 10), // ← YYYY-MM-DD
                    dataFim: dataFim.toISOString().substring(0, 10)  // ← YYYY-MM-DD
                }
            });
