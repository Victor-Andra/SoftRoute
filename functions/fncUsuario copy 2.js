        ...
        Usuario.findById(usuarioId)
            .then(usuario => {
                ...
                return Promise.all([
                    ...
                    // --- Nova lógica: pré-agrupar permissões habilitadas ---
                    ...
                    empresas.forEach(empresa => {
                        ...
                        usufuncs.forEach(func => {
                            const permissao = usupermis.find(p =>
                                p.usupermis_codfunc.toString() === func._id.toString() &&
                                p.usupermis_empresaid &&
                                p.usupermis_empresaid.toString() === empresa._id.toString()
                            );

                            if (permissao && parseInt(permissao.usupermis_tipo) >= 2) {
                                funcsDaEmpresa.push({
                                    usufunc_nome: func.usufunc_nome,
                                    usupermis_tipo: permissao.usupermis_tipo
                                });
                            }
                        });
                    ...
                    res.render('ferramentas/usuario/usuarioEdi', {
                        ...
                        empresas,
                        usufuncs,
                        permissoesHabilitadas, // ← novo dado pré-processado
                        base64Image
                    });
                });
            })
