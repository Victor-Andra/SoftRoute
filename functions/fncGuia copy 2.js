...
module.exports = {FiltroEvoatend,
    ...
    filtraGuialis_OLD31072026(req, res, resposta) {
        let db = req.cookies['preferredDb'];
    ...
    Agenda.find(agendaQuery)
        .then((agendas) => {
            ...
            // 📊 ESTATÍSTICAS E CONSOLIDADO
            const estatisticas = { qa:0, qt:0, qac:0, qtv:0, qtse:0, qace:0, atvo:0, qtva:0, qguiasUnicas: 0 };
            const guiasUnicasSet = new Set();
            const consolidadoTerapiasMap = new Map();

            agendas.forEach(a => {
                estatisticas.qa++;
                const ehCancelado = (a.agenda_categoria === "Feriado" || a.agenda_categoria === "Falta Absoluta");
                const temEvolucao = (a.agenda_evolucao && String(a.agenda_evolucao).trim() !== '');
                
                // ✅ CORREÇÃO ROBUSTA: Cria um objeto 'guia' seguro, nunca será undefined
                const guia = a.agenda_guia || {};
                const numGuia = guia.guia_num ? String(guia.guia_num).trim().toUpperCase() : '';
                const numSenha = guia.guia_senha ? String(guia.guia_senha).trim() : '';
                
                const temGuia = numGuia !== '';
                const temSenha = numSenha !== '';

                // Lógica de Guias Únicas
                if (temGuia) {
                    guiasUnicasSet.add(numGuia);
                }

                // Lógica de Consolidado por Terapia
                const terapiaId = a.agenda_terapiaid ? a.agenda_terapiaid.toString() : 'Sem Terapia';
                if (!consolidadoTerapiasMap.has(terapiaId)) {
                    consolidadoTerapiasMap.set(terapiaId, { total: 0, unicasSet: new Set() });
                }
                const teraData = consolidadoTerapiasMap.get(terapiaId);
                if (temGuia) {
                    teraData.total++;
                    teraData.unicasSet.add(numGuia);
                }
            ...
            estatisticas.qguiasUnicas = guiasUnicasSet.size;
            ...
            return Promise.all([
                ...
                // 🗂️ CRIAR MAPAS DE CONSULTA RÁPIDA
                const beneMap = {}, convMap = {}, terapiaMap = {}, usuarioMap = {}, salaMap = {}, horaageMap = {};
                bene.forEach(b => { beneMap[b._id.toString()] = b.bene_nome; });
                convs.forEach(c => { convMap[c._id.toString()] = c.conv_nome; });
                terapias.forEach(t => { terapiaMap[t._id.toString()] = t.terapia_nomecid; });
                ...
                agendas.forEach(a => {
                    const dataAgenda = new Date(a.agenda_data);
                    a.agenda_hora = dataAgenda.getUTCHours().toString().padStart(2,'0') + ':' + dataAgenda.getUTCMinutes().toString().padStart(2,'0');
                    a.agenda_data_dia = fncGeral.getDataFMT(dataAgenda);
                    ...
                    a.evolucaoSimNao = (a.agenda_evolucao?.trim()) ? 'Sim' : 'Não';
                    a.datacad = a.agenda_datacad ? fncGeral.getDataFMT(new Date(a.agenda_datacad)) : '-';
                    a.dataedi = a.agenda_dataedi ? fncGeral.getDataFMT(new Date(a.agenda_dataedi)) : '-';
                    
                    // ✅ Mapeamento seguro de usuários do Agendamento
                    const idCad = a.agenda_usucad ? String(a.agenda_usucad) : '';
                    const idEdi = a.agenda_usuedi ? String(a.agenda_usuedi) : '';
                    a.usuarioCadNome = usuarioMap[idCad] || 'Desconhecido';
                    a.usuarioEdiNome = usuarioMap[idEdi] || 'Desconhecido';

                    // ✅ Mapeamento seguro de usuários da GUIA (usando o objeto 'guia' blindado)
                    const guia = a.agenda_guia || {};
                    const idGuiaCad = guia.guia_usucad ? String(guia.guia_usucad) : '';
                    const idGuiaEdi = guia.guia_usuedi ? String(guia.guia_usuedi) : '';
                    
                    a.guiaUsuarioCadNome = usuarioMap[idGuiaCad] || 'Não informado';
                    a.guiaDatacad = guia.guia_datacad ? fncGeral.getDataFMT(new Date(guia.guia_datacad)) : 'Não inserida';
                    a.guiaUsuarioEdiNome = usuarioMap[idGuiaEdi] || 'Não informado';
                    a.guiaDataedi = guia.guia_dataedi ? fncGeral.getDataFMT(new Date(guia.guia_dataedi)) : '-';
                    
                    a.agenda_datasenha_input = guia.guia_senhadatacad ? new Date(guia.guia_senhadatacad).toISOString().split('T')[0] : '';
                    a.guia_numdatacad_input = guia.guia_numdatacad ? new Date(guia.guia_numdatacad).toISOString().split('T')[0] : '';
